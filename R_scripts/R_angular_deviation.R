...
tempData <- Complete[-grep("(AB|AB[ap]|EMS|P[1-2])$",Complete$Cell),c("pOV","pLR","pDV","pAP", "aMean","StartTime","EndTime","Cell","embryo","Divided")]
...
tempData <- tempData[order(tempData$EndTime),] # variables measured at anaphase onset

#get cumulative deviation for each embryo ####
deviation <- by(tempData$aMean,tempData$embryo, function(x) cumsum(na.omit(x)))
...
endtimes <- by(tempData[,c("aMean","EndTime")], tempData$embryo, function(x) x$EndTime[!is.na(x$aMean)], simplify = T)
...
#plot raw curves ####
pdf(paste0(outDir,today,"_angular_divergence_raw.pdf"), width=4.5,height = 4.5, pointsize = 10)
{
  #plot deviations
  par(mfcol=c(1,1), mar=c(2.5,3.5,2.1,1.1), mgp=c(1.5,0.5,0), cex.lab=0.8, cex.axis=0.80, cex.main=1)
  plot(x=0,y=0,
    main="Cumulative angular divergence",
    type="n",
    #bty="n",
    #log="y",
    #yaxt="n",
    ylab="Cummulative deviation from \n cell division orientation in controls [um]",
    xlab="Time [min]",
    xlim=c(25,200),
    ylim=c(5,max(maxDev))
  )
...
#remap values so that they can be coerced to the matrix ####
  ...
  #plot raw curves ####
pdf(paste0(outDir,today,"_angular_divergence_raw_normalized.pdf"), width=4.5,height = 4.5, pointsize = 10)
  {
    #plot deviations
    par(mfcol=c(1,1), mar=c(2.5,3.5,2.1,1.1), mgp=c(1.5,0.5,0), cex.lab=0.8, cex.axis=0.80, cex.main=1)
    plot(x=0,y=0,
         main="Normalized angular divergence",
         type="n",
         #bty="n",
         #log="y",
         #yaxt="n",
         ylab="Angular deviation from division orientation in controls [um]",
         xlab="Time [min]",
         xlim=c(20,200),
         ylim=c(5,max(devMat.norm, na.rm = T))
    )
    
    # plot individual embryos
    # wt c("GZ01", "GZ02", "GZ03", "GZ04", "GZ05", "GZ06", "GZ07", "GZ08", "GZ09", "GZ10")
    for(id in row.names(devMat.norm)){
      vect <-  devMat.norm[id,]
      l <- lines(names(vect),vect, col=ifelse(id %in% outliers,"black",pal[Embs[id,"Group"]]), )
    }
    dev.off()
  }
...
pdf(paste0(outDir,today,"_angular_divergence_normalized.pdf"), width=3.5,height = 3.5, pointsize = 10)
{
...
#plot averages
  ...
  matplot(as.numeric(rownames(b)) , b[,"s"] + outer(b[,"sem"], c(0,1,-1)), type="l", 
          ...
          # log="y",
          xlab ="Time [um]", 
          ylab="Mean angular deviation per cell [um]")
